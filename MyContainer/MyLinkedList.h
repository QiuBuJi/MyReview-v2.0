...
public:
	...
	//重载运算符 << 添加到尾
	CMyLinkedList<Type>& operator<<(const Type &data){
		Append(data);
		return *this;
	}
	//重载运算符 >> 添加到首
...
template<class Type>
inline CMyLinkedList<Type>::CMyLinkedList()
{
...
template<class Type>
inline bool CMyLinkedList<Type>::Append(const Type &Data)
{
...
template<class Type>
inline bool CMyLinkedList<Type>::AddFirst(const Type &Data)
{
	CNod<Type> *temp = new CNod<Type>(Data);//初始化为“Data-数据”。
...
template<class Type>
inline bool CMyLinkedList<Type>::Insert(ULONG Index, const Type &Data)
{
...
template<class Type>
inline bool CMyLinkedList<Type>::Insert(const Type &Data, Limit Fist_Or_Last /*= Last*/)
{
	bool state = false;
	switch (Fist_Or_Last)
	{
...
template<class Type>
inline void CMyLinkedList<Type>::Clear()
{
...
template<class Type>
inline bool CMyLinkedList<Type>::Delete(ULONG Index)
{
	CNod<Type> *temp = Home;//初始化为“Home-首数据”。
...
template<class Type>
bool CMyLinkedList<Type>::DeleteLast()
{
...
template<class Type>
bool CMyLinkedList<Type>::DeleteFirst()
{
...
template<class Type>
inline bool CMyLinkedList<Type>::Delete(Limit Fist_Or_Last /*= Last*/)
{
	bool state = false;
	switch (Fist_Or_Last)
	{
...
template<class Type>
inline CNod<Type>* CMyLinkedList<Type>::GetNode(ULONG Index)
{
...
template<class Type>
inline Type CMyLinkedList<Type>::GetData(ULONG Index)
{
	CNod<Type> *Nod = GetNode(Index);
	if (!Nod)//如果节点为空，抛出异常。
		throw 1;
...
template<class Type>
inline Type CMyLinkedList<Type>::GetData(Limit Fist_Or_Last /*= Last*/)
{
	Type data;
	switch (Fist_Or_Last)
	{
...
template<class Type>
inline ULONG CMyLinkedList<Type>::IndexOf(const Type &Data, ULONG StartIndex)
{
	CNod<Type> *temp = Home;//初始化为“Home-首数据”。
...
template<class Type>
inline bool CMyLinkedList<Type>::Replace(ULONG Index, const Type &Data)
{
	CNod<Type> *nod = GetNode(Index);
	if (!nod)//没找到节点
		return false;
...
template<class Type>
inline ULONG CMyLinkedList<Type>::Length(){
	return Sum;
}

template<class Type>
inline bool CMyLinkedList<Type>::Swap(ULONG Index1, ULONG Index2)
{
...
template<class Type>
inline bool CMyLinkedList<Type>::SaveToFile(TCHAR*FileName)
{
	FILE *file = NULL;
...
template<class Type>
inline bool CMyLinkedList<Type>::ReadFromFile(TCHAR*FileName)
{
	FILE *file = NULL;
